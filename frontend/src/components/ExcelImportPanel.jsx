...
const IMPORT_FIELDS = {
  products: [
    ...
    { key: 'sku', label: 'SKU / Mã sản phẩm', required: true },
...
const TEMPLATE_ROWS = {
  ...
  invoices: [
    {
      'Mã đơn hàng': 'HDIMPORT001',
      'Mã khách hàng': 'KHLE001',
    ...
    {
      'Mã đơn hàng': 'HDIMPORT002',
      'Mã khách hàng': 'KHSI001',
...
export default function ExcelImportPanel({
  ...
  const commitImport = () => runAction('commit', async () => {
    ...
    if (!window.confirm(`Import ${selected.length} dòng đã chọn? Backend sẽ validate lại trước khi ghi dữ liệu.`)) return null;

export const PAPER_DIMENSIONS_MM = Object.freeze({
  ...
  K57: { width: 57, height: 220, label: 'K57' },
});
...
export const DEFAULT_INVOICE_TEMPLATE_SETTINGS = Object.freeze({
  ...
  headerLogoHeightMm: 18,
  storeName: '',
  storeAddress: '',
  storePhone: '',
});

const BOOLEAN_KEYS = new Set(['showLogo', 'showQr', 'showSignature', 'showNote', 'showSubtotal', 'showDiscount', 'showDelivery', 'showGrandTotal', 'showOldDebt', 'showPayable', 'showPaid', 'showDebt', 'showChange', 'tableBorder']);
...
export function getPaperDimensions(paperSize = 'A5', orientation = 'portrait') {
  const requestedPaper = String(paperSize || 'A5').toUpperCase();
  const normalizedPaper = requestedPaper === 'K58' ? 'K57' : requestedPaper;
  const base = PAPER_DIMENSIONS_MM[normalizedPaper] || PAPER_DIMENSIONS_MM.A5;
  if (normalizedPaper === 'K80' || normalizedPaper === 'K57') {
    return { ...base, label: `${base.label} dọc`, paperSize: normalizedPaper, orientation: 'portrait' };
  }
...
export function normalizeTemplateSettings(input = {}) {
  ...
  const merged = {
    ...
    showSubtotal: pickFirst(rawSettings.showSubtotal, rawSettings.show_subtotal, flags.showSubtotal, content.showSubtotal),
    showDiscount: pickFirst(rawSettings.showDiscount, rawSettings.show_discount, flags.showDiscount, content.showDiscount),
    showDelivery: pickFirst(rawSettings.showDelivery, rawSettings.show_delivery, flags.showDelivery, content.showDelivery),
    showGrandTotal: pickFirst(rawSettings.showGrandTotal, rawSettings.show_grand_total, flags.showGrandTotal, content.showGrandTotal),
    showOldDebt: pickFirst(rawSettings.showOldDebt, rawSettings.show_old_debt, flags.showOldDebt, content.showOldDebt),
    showPayable: pickFirst(rawSettings.showPayable, rawSettings.show_payable, flags.showPayable, content.showPayable),
    showPaid: pickFirst(rawSettings.showPaid, rawSettings.show_paid, flags.showPaid, content.showPaid),
    showDebt: pickFirst(rawSettings.showDebt, rawSettings.show_debt, flags.showDebt, content.showDebt),
    showChange: pickFirst(rawSettings.showChange, rawSettings.show_change, flags.showChange, content.showChange),
  ...
  const requestedPaperSize = String(result.paperSize || '').toUpperCase();
  result.paperSize = ['A4', 'A5', 'K80', 'K57', 'K58'].includes(requestedPaperSize) ? (requestedPaperSize === 'K58' ? 'K57' : requestedPaperSize) : 'A5';
  result.orientation = result.paperSize === 'K80' || result.paperSize === 'K57' ? 'portrait' : (result.orientation === 'landscape' ? 'landscape' : 'portrait');
  result.showQr = false;
  ...
  return {
    layout_json: {
      ...
      content: {
        ...
        showDelivery: normalized.showDelivery,
        showGrandTotal: normalized.showGrandTotal,
        showOldDebt: normalized.showOldDebt,
        showPayable: normalized.showPayable,
        showPaid: normalized.showPaid,
        showDebt: normalized.showDebt,
        showChange: normalized.showChange,
        showFooter: true,
      },
      table: {

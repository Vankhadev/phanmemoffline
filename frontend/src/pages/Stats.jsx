...
export default function Stats() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      if (!statsResponse.ok) throw new Error(statsData?.error || 'Không thể tải báo cáo doanh thu.');
      if (!profitResponse.ok) throw new Error(profitData?.error || 'Không thể tải lợi nhuận ước tính.');
    ...
    <div>
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {[['day', 'Hôm nay'], ['week', 'Tuần'], ['month', 'Tháng'], ['year', 'Năm']].map(([key, label]) => (
            <button
              key={key}
              type="button"
      ...
      <div className="card mb-4 border border-green-200 bg-gradient-to-r from-green-50 to-blue-50">
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <button type="button" onClick={exportCurrentRangeReport} className="flex items-center gap-1 rounded bg-purple-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-purple-700">
            <FileDown size={16} /> Xuất khoảng đang xem
          </button>
        </div>
      </div>

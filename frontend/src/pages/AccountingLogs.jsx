      ...
      <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="accounting-log-detail-title">
        <div className="sticky top-0 flex items-start justify-between gap-3 border-b border-gray-100 bg-white px-5 py-4">
          <div>
            ...
            <h2 id="accounting-log-detail-title" className="mt-1 text-lg font-bold text-gray-900">{log?.content || 'Nhật ký hoạt động'}</h2>
          </div>
          <button type="button" onClick={onClose} className="rounded-lg border border-gray-200 p-2 text-gray-400 hover:bg-gray-50 hover:text-gray-700" aria-label="Đóng"><X size={18} /></button>
        </div>
        <div className="space-y-4 p-5">
          {loading ? (
            <div className="flex min-h-44 items-center justify-center gap-2 text-gray-500"><Loader2 size={22} className="animate-spin" /> Đang tải chi tiết...</div>
          ) : error ? (
            <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>
          ) : log ? (
            <>
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div className="rounded-xl border border-gray-200 bg-gray-50 p-3"><div className="text-xs text-gray-500">Người thực hiện</div><div className="mt-1 font-bold text-gray-800">{log.user_name || `Người dùng #${log.user_id || '—'}`}</div></div>
              ...
              <div><div className="mb-1 text-sm font-bold text-gray-700">Nội dung</div><div className="rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm text-blue-800">{log.content || '—'}</div></div>
              <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
                <div><div className="mb-1 text-sm font-bold text-gray-700">Dữ liệu trước</div><pre className="max-h-72 overflow-auto whitespace-pre-wrap break-words rounded-xl bg-slate-950 p-3 text-xs text-slate-200">{JSON.stringify(log.before, null, 2) || 'null'}</pre></div>
...
export default function AccountingLogs() {
  ...
  const loadLogs = useCallback(async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(requestError?.data || requestError, requestError?.message || 'Không thể tải nhật ký hoạt động.'));
    } finally {
      setLoading(false);
    }
  }, [filters, page]);
  ...
  async function openDetail(row) {
    ...
    try {
      ...
      setDetailError(getApiErrorMessage(requestError?.data || requestError, requestError?.message || 'Không thể tải chi tiết nhật ký.'));
    } finally {
    ...
    <div className="min-w-0 space-y-4">
      <section className="overflow-hidden rounded-2xl border border-indigo-900 bg-gradient-to-r from-indigo-950 via-slate-900 to-purple-950 text-white shadow-lg">
        <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="text-xs font-bold uppercase tracking-[0.22em] text-indigo-200/80">Kế toán · Kiểm soát</div>
              <h1 className="mt-1 text-2xl font-bold">Nhật ký hoạt động</h1>
              <p className="mt-1 max-w-3xl text-sm text-indigo-100/75">Tra cứu người thực hiện, thời gian, hành động, đối tượng và nội dung thay đổi của nghiệp vụ kế toán.</p>
            </div>
          </div>
          <button type="button" onClick={loadLogs} disabled={loading} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-white/20 bg-white/10 px-4 py-2.5 text-sm font-bold hover:bg-white/20 disabled:opacity-60"><RefreshCw size={17} className={loading ? 'animate-spin' : ''} /> Làm mới</button>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
        <form onSubmit={applyFilters} className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-[150px_150px_210px_minmax(240px,1fr)_auto]">
          <div><label className="mb-1 block text-xs font-semibold text-gray-500">Từ ngày</label><input type="date" className="input-field" value={draft.from} max={draft.to || undefined} onChange={event => setDraft(current => ({ ...current, from: event.target.value }))} /></div>
          <div><label className="mb-1 block text-xs font-semibold text-gray-500">Đến ngày</label><input type="date" className="input-field" value={draft.to} min={draft.from || undefined} onChange={event => setDraft(current => ({ ...current, to: event.target.value }))} /></div>
          ...
          <div><label className="mb-1 block text-xs font-semibold text-gray-500">Tìm kiếm</label><div className="relative"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" /><input className="input-field pl-9" value={draft.search} onChange={event => setDraft(current => ({ ...current, search: event.target.value }))} placeholder="Nội dung, người thực hiện, entity, mã..." /></div></div>
          <div className="flex items-end gap-2"><button type="submit" disabled={loading} className="btn-primary min-h-11 flex-1 xl:flex-none"><Filter size={16} /> Lọc</button><button type="button" onClick={resetFilters} disabled={loading} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-500 hover:bg-gray-50" title="Đặt lại"><RefreshCw size={16} /></button></div>
        </form>
        <div className="mt-3 flex flex-wrap gap-2 text-xs text-gray-500">
          <span className="inline-flex items-center gap-1 rounded-full border border-indigo-100 bg-indigo-50 px-2.5 py-1 font-semibold text-indigo-700"><CalendarDays size={13} /> {filters.from || 'Từ đầu'} - {filters.to || 'Hiện tại'}</span>
          <span>{data.pagination.total.toLocaleString('vi-VN')} nhật ký</span>
          {data.generated_at && <span>Truy vấn lúc: {formatDateTime(data.generated_at)}</span>}
        </div>
      ...
      <section className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        ...
        {loading ? <div className="flex min-h-44 items-center justify-center gap-2 border-t border-gray-100 text-gray-500"><Loader2 size={24} className="animate-spin text-indigo-500" /> Đang tải nhật ký...</div> : data.items.length === 0 ? <div className="border-t border-gray-100 px-4 py-14 text-center text-gray-400"><div className="mb-2 text-4xl opacity-30">🕘</div><div className="font-semibold text-gray-500">Không có nhật ký phù hợp</div></div> : null}

...
Khi phát hành version `1.3.8`, release cần tối thiểu các asset:

- `banhangoffline-setup-v1.3.8-x64.exe`
- `banhangoffline-setup-v1.3.8-x64.exe.blockmap`
- `banhangoffline-setup-v1.3.8-ia32.exe`
- `banhangoffline-setup-v1.3.8-ia32.exe.blockmap`
- `latest.yml`
...
```cmd
gh release create v1.3.8 release/banhangoffline-setup-v1.3.8-x64.exe release/banhangoffline-setup-v1.3.8-x64.exe.blockmap release/banhangoffline-setup-v1.3.8-ia32.exe release/banhangoffline-setup-v1.3.8-ia32.exe.blockmap release/latest.yml release/update-manifest.json --title "BanHangOffline v1.3.8" --generate-notes --latest
```

Nếu release đã tồn tại, dùng `gh release upload v1.3.8 ... --clobber` có chủ đích sau khi kiểm tra file đúng.

## Kiểm tra remote sau khi publish

```cmd
powershell -ExecutionPolicy Bypass -File scripts/verify-windows-release.ps1 -Mode remote -Version 1.3.8 -Tag v1.3.8 -Arch x64,ia32
```
...
- Máy Windows 10/11 thông thường: tải `banhangoffline-setup-v1.3.8-x64.exe`.
- Máy Windows 32-bit hoặc báo lỗi “Ứng dụng này không thể chạy trên PC của bạn”: tải `banhangoffline-setup-v1.3.8-ia32.exe`.
- Chỉ tải từ GitHub Release chính thức của repo `Vankhadev/phanmemoffline`.
- Nếu trình duyệt/SmartScreen/antivirus cảnh báo, kiểm tra tên file và nguồn tải trước. Không chạy file nếu tên không đúng `banhangoffline-setup-v1.3.8-x64.exe` hoặc `banhangoffline-setup-v1.3.8-ia32.exe`.
...
- Nếu release lỗi, tạo tag và release mới thay thế, ví dụ `v1.3.8`.

...
## Bảng phát hành chính thức v1.5.0
...
| Trạng thái | Đã chuẩn bị source/metadata, chờ commit/tag và GitHub Actions publish |

### Ghi chú thay đổi tổng quan

Bản 1.5.0 bổ sung nền tảng kế toán, báo cáo thuế GTGT, báo cáo tồn kho, nhật ký hoạt động, phân quyền kế toán/thu ngân/nhân viên và backup database trước migration kế toán.

### Phát hành

- Đồng bộ version 1.5.0 trong package root, backend, frontend và các lockfile tương ứng.
- Cập nhật changelog, tài liệu release, manifest ví dụ, tên asset và tag kiểm tra theo v1.5.0.
...
- Giữ nguyên metadata generated production nếu chưa có installer v1.5.0 để sinh SHA/size chính xác.

### Lưu ý quan trọng cho người dùng

- Windows 10/11 64-bit: dùng `banhangoffline-setup-v1.5.0-x64.exe`.
- Windows 32-bit hoặc máy báo “Ứng dụng này không thể chạy trên PC của bạn”: dùng `banhangoffline-setup-v1.5.0-ia32.exe`.
...
- Nên backup dữ liệu runtime trước khi cập nhật/cài đặt phiên bản mới; bản này cũng tự tạo backup trước migration schema kế toán.

## Asset GitHub Release bắt buộc

Khi phát hành version `1.5.0`, release cần tối thiểu các asset:

- `banhangoffline-setup-v1.5.0-x64.exe`
- `banhangoffline-setup-v1.5.0-x64.exe.blockmap`
- `banhangoffline-setup-v1.5.0-ia32.exe`
- `banhangoffline-setup-v1.5.0-ia32.exe.blockmap`
- `latest.yml`
...
```cmd
gh release create v1.5.0 release/banhangoffline-setup-v1.5.0-x64.exe release/banhangoffline-setup-v1.5.0-x64.exe.blockmap release/banhangoffline-setup-v1.5.0-ia32.exe release/banhangoffline-setup-v1.5.0-ia32.exe.blockmap release/latest.yml release/update-manifest.json --title "BanHangOffline v1.5.0" --generate-notes --latest
```

Nếu release đã tồn tại, dùng `gh release upload v1.5.0 ... --clobber` có chủ đích sau khi kiểm tra file đúng.

## Kiểm tra remote sau khi publish

```cmd
powershell -ExecutionPolicy Bypass -File scripts/verify-windows-release.ps1 -Mode remote -Version 1.5.0 -Tag v1.5.0 -Arch x64,ia32
```
...
- Máy Windows 10/11 thông thường: tải `banhangoffline-setup-v1.5.0-x64.exe`.
- Máy Windows 32-bit hoặc báo lỗi “Ứng dụng này không thể chạy trên PC của bạn”: tải `banhangoffline-setup-v1.5.0-ia32.exe`.
- Chỉ tải từ GitHub Release chính thức của repo `Vankhadev/phanmemoffline`.
- Nếu trình duyệt/SmartScreen/antivirus cảnh báo, kiểm tra tên file và nguồn tải trước. Không chạy file nếu tên không đúng `banhangoffline-setup-v1.5.0-x64.exe` hoặc `banhangoffline-setup-v1.5.0-ia32.exe`.
...
- Nếu release lỗi, tạo tag và release patch mới thay thế, ví dụ `v1.5.1`.

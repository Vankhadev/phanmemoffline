...
// Mojibake cp1252: Ã/Ä/Æ/Â followed by a byte-high char (0x80-0xFF) or specific letter => the pair encodes a vietnamese letter.
// Also áº/á» (U+00E1 + U+00BA/BB), and sequences like â€ (U+00E2 + U+20AC).
function hasMojibake(L){
  for(let i=0;i<L.length;i++){
    const cp=L.charCodeAt(i), next=L.charCodeAt(i+1);
    if((cp===0xC3||cp===0xC4||cp===0xC6||cp===0xC2) && ((next>=0x80&&next<=0xFF)||(next>=0x41&&next<=0x7A)||(next>=0x61&&next<=0x7A))) return true;
    if(cp===0xE1 && (next===0xBA||next===0xBB)) return true;
    if(cp===0xE2 && next===0x20AC) return true;
    if(cp===0xC3 && next===0x83) return true; // Ãƒ (double)
  }
  return false;
}

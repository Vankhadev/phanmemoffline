...
const markers=['áº','á»','Ã','Ä','Æ','Æ°','â€','Â€','Â‚','Âƒ','Â„'];
function hasMojibake(L){ for(const m of markers){ if(L.includes(m)) return true; } return false; }
...
for(const file of walk(ROOT)){
  ...
  for(let i=0;i<lines.length;i++){
    ...
    if(r){ const dec=r.toString('utf8'); if(dec!==lines[i]){ lines[i]=dec; changed=true; lc++; } else failed++; }
    else failed++;
  }
  if(changed){ fs.writeFileSync(file,lines.join('\n'),'utf8'); totalFiles++; totalLines+=lc; rep.push(`${path.relative(ROOT,file)}: ${lc}`); }
}
console.log(`Files: ${totalFiles} Lines decoded: ${totalLines} failed/unchanged: ${failed}`);
